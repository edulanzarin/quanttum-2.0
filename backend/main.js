...
function gerenciarBancos(banco, caminho_pdf) {
    return new Promise((resolve, reject) => {
        execFile(
            'python',
            [
...
ipcMain.handle("gerenciar-bancos", async (event, banco, caminho_pdf) => {
    try {
        const result = await gerenciarBancos(banco, caminho_pdf);
        return result;
    } catch (error) {
        return {success: false, message: error};
    }
